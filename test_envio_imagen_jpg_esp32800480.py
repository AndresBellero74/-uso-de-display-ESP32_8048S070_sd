...
async def test():
    ...
    async with websockets.connect('ws://192.168.0.200:8888') as websocket:
        count = 0
        while count < 5000:
            ...
            #new_im.save('foto.jpg',"JPEG",dpi=[90,90],quality=15)
            new_im.save('foto.jpg', "JPEG",dpi=[90,90],quality=40)
            ...
            archivo.close()
            veces = int(longitud / 50000 + 1)
            # reabro para poder leerlo de a bloques
            archivo = open("foto.jpg", "rb")
            vueltas = 1
            while vueltas <= veces:
                data = archivo.read(50000)
